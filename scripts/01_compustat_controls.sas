...
data mna_media; 
	set DAT.mna_03_18_media_ctr; 
run;

proc import datafile="&data.\compustat_data.csv"
	dbms=csv 
	out=comp_data 
	replace; 
run;
...
data mna_media_comp;
	...
	if missing(ta_oibdp) then delete;
...
data mna_media_comp;
	...
	ac_loss = .;
	ta_loss = .;
	if not missing(ac_ib) then ac_loss = (ac_ib < 0);
	if not missing(ta_ib) then ta_loss = (ta_ib < 0);
