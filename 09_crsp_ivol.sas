...
%let out_file = C:\\Users\\¤ý«FÒj\\Desktop\\RA\\Kim_Cha\\results\\mna_media_ivol.csv;
...
proc import datafile="C:\Users\\¤ý«FÒj\\Desktop\\RA\\Kim_Cha\\Data\\data\\FF3.csv"
	dbms=csv 
	out=ff3_data
	replace;
run;
...
proc sql;
	...
	on a.t_permno = b.permno and b.date between intnx('day', a.d_ann, -&K) and intnx('day', a.d_ann, -60)
...
libname outlib "&out";
data outlib.mna_media_ivol;
    set mna_media_ivol;
run;

proc export data=outlib.mna_media_ivol
    outfile="&out_file"
    dbms=csv
    replace;
    putnames=yes;
